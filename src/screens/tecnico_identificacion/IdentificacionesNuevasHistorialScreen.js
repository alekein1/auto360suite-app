...
// Screen
// =========================
export default function IdentificacionesNuevasHistorialScreen({ navigation }) {
  ...
  async function cargarIdentificaciones() {
    try {
      ...
      const res = await fetch(`${API}/identificacion-nueva/listar?estado=todos`, {
      ...
      console.log(e);
      Alert.alert("Error", "Error cargando identificaciones nuevas");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function verPDF(id_orden) {
    try {
      ...
      const fileUri =
        ...
        `identificacion_nueva_${id_orden}_${Date.now()}.pdf`;
  ...
  function editarOrden(id_orden) {
    ...
    navigation.navigate("IdentificacionNueva", { id_orden });
  }
  ...
  if (loading) {
    return (
      <View style={styles.center}>
        ...
        <Text style={{ marginTop: 10 }}>Cargando identificaciones nuevas...</Text>
      </View>
    );
  }
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={{ paddingBottom: 20 }}>
        <View style={styles.card}>
          ...
          <View style={styles.cardHeader}>
            <View style={{ flex: 1 }}>
              <Text style={styles.cardHeaderTitle}>📄 Identificación Vehicular Nueva</Text>
          ...
          <View style={{ padding: 12 }}>
            {datosPagina.length === 0 ? (
              ...
              datosPagina.map((r, idx) => {
                ...
                const finalizada = String(r?.estado_orden || "").toLowerCase() === "finalizada";
                ...
                return (
                  <View key={`${r?.id_orden}-${idx}`} style={styles.itemCard}>
                    ...
                    <View style={styles.actionsRow}>
                      ...
                      {esAdminIdentificacion && finalizada && (
                        <TouchableOpacity
